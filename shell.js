...
var Module = {
  ...
  '_ptrs': [],
  'API_VERSION': 1,
  'camera_buffer_opengl_texture': 0,
  'device': {
    'analog': 5,
    'joypad': 1,
    'keyboard': 3,
  ...
  'get_system_info': function () {
    ...
    var obj = {
      library_name: this.Pointer_stringify(this.getValue(_data, '*')),
      library_version: this.Pointer_stringify(this.getValue(_data + 4, '*')),
      valid_extensions: this.Pointer_stringify(this.getValue(_data + 8, '*')),
  ...
  'serialize': function () {
    ...
    if (this._retro_serialize(_data, size)) {
      data = new Uint8Array(this.HEAP8.buffer, _data, size).slice(0)
    }
    this._free(_data)
    return data
  },
  ...
  'set_info': function (_info, info) {
    if (info.path) {
      var path = this._malloc(info.path.length + 1)
      this._ptrs.push(path)
      this.writeStringToMemory(info.path, path)
      this.setValue(_info, path, '*')
    }
    if (info.meta) {
      var meta = this._malloc(info.meta.length + 1)
      this._ptrs.push(meta)
      this.writeStringToMemory(info.meta, meta)
      this.setValue(_info + 12, meta, '*')
    }
    ...
    this._retro_set_environment(Runtime.addFunction(function (fn, cmd, _data) {
      switch (cmd) {
        ...
        case this.environment.get_system_directory:
          var str = fn(cmd)
          var buffer = this._malloc(str.length + 1)
          this._ptrs.push(buffer)
          this.writeStringToMemory(str, buffer)
          this.setValue(_data, buffer, '*')
          return true
        ...
        case this.environment.set_support_no_game:
          return fn(cmd, this.getValue(_data, 'i8'))
        case this.environment.get_libpath:
          var str = this.environment(cmd)
          var buffer = this._malloc(str.length + 1)
          this._ptrs.push(buffer)
          this.writeStringToMemory(str, buffer)
          this.setValue(_data, buffer, '*')
        ...
        case this.environment.get_input_device_capabilities:
          this.setValue(_data, fn(cmd), 'i64')
          return true
